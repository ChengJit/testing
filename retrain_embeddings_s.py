#!/usr/bin/env python3
"""
INSIGHTFACE-ACCELERATED Face Recognition Training for Jetson (BUFFALO_S)
- Generates 'known_embeddings.npz' (compatible with buffalo_s model)
- FULLY COMPATIBLE with inventory_door_cctv.py using buffalo_s
...
def get_insightface_app(use_gpu=True):
    """Get InsightFace app (Buffalo_S for speed - MATCHES CCTV SCRIPT)"""
    try:
        ...
        # IMPORTANT: Use buffalo_s to match CCTV script (changed from buffalo_l)
        print("⚡ Using Buffalo_S model for SPEED (matches CCTV script)")
        app = FaceAnalysis(
            name='buffalo_s',  # CHANGED FROM buffalo_l
            root='./models',
            providers=providers
        )
        
        # Match CCTV detection size (160x160)
        app.prepare(ctx_id=ctx_id, det_size=(160, 160))
        print(f"✅ InsightFace (Buffalo_S) loaded successfully with det_size=(160, 160)")
        return app
        
    except Exception as e:
        print(f"❌ Failed to initialize InsightFace: {e}")
        print(f"   Make sure buffalo_s model is downloaded")
        return None
...
def save_embeddings(embeddings_file, encodings, names):
    ...
    try:
        ...
        print(f"      📊 Embedding shape: {encodings_array[0].shape if len(encodings) > 0 else 'N/A'}")
        return True
    except Exception as e:
...
def train_embeddings():
    ...
    print(f"   Persons: {', '.join(person_folders)}")
    
    # GPU Selection
    use_gpu = input("\nUse GPU acceleration? (Y/n): ").strip().lower() != 'n'
    
    # Init App
    face_app = get_insightface_app(use_gpu)
    if not face_app: return False
    
    # Setup Data - MATCHES CCTV SCRIPT FILENAME
    output_file = "known_embeddings.npz"  # MATCHES CCTV SCRIPT DEFAULT
    ...
    if os.path.exists(output_file):
        ...
        print(f"ℹ️  Backed up existing file to {backup_file}")

    print(f"\n🎯 STARTING TRAINING -> {output_file}")
    print(f"   Model: Buffalo_S (FAST)")
    print(f"   Detection Size: 160x160")
    print("="*70)
    ...
    if batch_data['encodings']:
        ...
        print(f"   🎯 Model: Buffalo_S (compatible with CCTV script)")
        print("="*70)
        return True
    else:
...
def check_existing_embeddings():
    ...
    try:
        ...
        if len(encodings) > 0:
            print(f"   Embedding Dimensions: {encodings[0].shape}")
            print(f"   Expected for Buffalo_S: (512,)")
            
            if encodings[0].shape == (512,):
                print(f"   ✅ Correct shape for Buffalo_S")
            else:
                print(f"   ⚠️  Unexpected shape - may be from different model")
...
def main():
    print("\n" + "="*70)
    print("   INSIGHTFACE TRAINER - BUFFALO_S (FAST MODE)")
    print("   Compatible with inventory_door_cctv.py (buffalo_s model)")
    print("="*70)
    ...
    while True:
        ...
        print("   MENU")
        print("="*50)
        print("1. Train New Embeddings (Buffalo_S)")
