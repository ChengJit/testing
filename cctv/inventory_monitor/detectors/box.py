...
class BoxDetector:
    ...
    def reload_model(self, model_path: str):
        """Hot-swap the custom box detection model with a newly trained one."""
        try:
            from ultralytics import YOLO
            self.custom_model = YOLO(model_path)
            logger.info(f"Reloaded custom box model: {model_path}")
        except Exception as e:
            logger.error(f"Failed to reload box model: {e}")
...
class TrainingDataCollector:
    ...
    def __init__(
        ...
        # Retraining subprocess handle
        self._retrain_process = None
        self._retrain_model_output: Optional[str] = None
    ...
    def _process_sample(
        ...
        # Auto-verification: all detections must be yolo_custom with high confidence
        verified = False
        verification_method = None
        if box_detections and all(
            m == "yolo_custom" for m in methods
        ) and all(
            c >= self.auto_verify_threshold for c in confidences
        ):
            verified = True
            verification_method = "auto_confidence"
    ...
    def count_verified_samples(self) -> int:
        """Count verified samples in the manifest."""
        if not self._manifest_path.exists():
            return 0
        count = 0
        try:
            ...
            logger.error(f"Failed to count verified samples: {e}")
        return count

    def trigger_retrain(self, model_output_dir: str = "models") -> bool:
        """
        Trigger box model retraining if enough verified samples exist.

        Args:
            model_output_dir: Directory to save the new model

        Returns:
            True if retraining was started, False otherwise
        """
        import subprocess
        ...
        verified_count = self.count_verified_samples()
        if verified_count < self.retrain_min_samples:
            logger.info(
                f"Not enough verified samples for retraining: "
                f"{verified_count}/{self.retrain_min_samples}"
            )
            return False

        # Generate dataset.yaml from verified manifest entries
        dataset_dir = self.output_dir / "retrain_dataset"
        dataset_dir.mkdir(parents=True, exist_ok=True)

        dataset_yaml = dataset_dir / "dataset.yaml"
        dataset_config = {
            "path": str(dataset_dir.resolve()),
            "train": str(self._images_dir.resolve()),
            "val": str(self._images_dir.resolve()),
            "names": {0: "box"},
        }
        ...
        try:
            ...
            with open(dataset_yaml, "w") as f:
                ...
                f.write(f"train: {self._images_dir.resolve()}\n")
                f.write(f"val: {self._images_dir.resolve()}\n")
                f.write("names:\n  0: box\n")

        output_path = Path(model_output_dir)
        output_path.mkdir(parents=True, exist_ok=True)
        self._retrain_model_output = str(output_path / "retrained_box_model")
        ...
        try:
            self._retrain_process = subprocess.Popen(
                [
                    ...
                    "epochs=50",
                    "imgsz=480",
                    f"project={self._retrain_model_output}",
                    "name=train",
                ],
            ...
            logger.info(
                f"Box model retraining started with {verified_count} verified samples"
            )
            return True
        except Exception as e:
    ...
    def check_retrain_complete(self) -> Optional[str]:
        """
        Check if retraining subprocess has completed.

        Returns:
            Path to new model weights if complete and successful, else None
        """
        ...
        if retcode == 0:
            # Look for best.pt in the output directory
            best_path = Path(self._retrain_model_output) / "train" / "weights" / "best.pt"
            if best_path.exists():
                logger.info(f"Retraining complete, new model: {best_path}")
                self._retrain_process = None
                return str(best_path)
            else:
                logger.warning(f"Retraining completed but best.pt not found at {best_path}")
        else:
            stderr = self._retrain_process.stderr.read().decode() if self._retrain_process.stderr else ""
            logger.error(f"Retraining failed (exit code {retcode}): {stderr[:500]}")
