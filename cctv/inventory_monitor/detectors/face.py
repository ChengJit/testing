...
class FaceRecognizer:
    ...
    def __init__(
        ...
        self._auto_register_counter = 0
        self._auto_registrations_path = Path("data") / "auto_registrations.jsonl"
        ...
        # Load auto-registration counter from existing records
        self._load_auto_register_counter()
        ...
        try:
            ...
            if self.known_embeddings is not None and len(self.known_names) > 0:
                ...
                if best_score >= self.recognition_threshold:
                    # Check for low-margin match (best within 0.1 of second-best)
                    if len(similarities) > 1:
                        sorted_scores = np.sort(similarities)[::-1]
                        margin = sorted_scores[0] - sorted_scores[1]
                        if margin < 0.1:
                            logger.warning(
                                f"Low-margin face match: {self.known_names[best_idx]} "
    ...
    def _load_auto_register_counter(self):
        """Load counter from existing auto_registrations.jsonl."""
        if self._auto_registrations_path.exists():
            try:
                count = 0
                with open(self._auto_registrations_path, "r") as f:
                    for line in f:
                        line = line.strip()
                        if line:
                            count += 1
                self._auto_register_counter = count
                logger.info(f"Auto-register counter loaded: {count}")
            except Exception as e:
                logger.warning(f"Failed to load auto-registration log: {e}")
                self._auto_register_counter = 0

    def auto_register(self, track_id: int, pattern: str = "Person_{}") -> Optional[str]:
        """
        Auto-register a face with a generated label.

        Args:
            track_id: Track ID with collected samples
            pattern: Label pattern with {} placeholder for counter

        Returns:
            Generated label if successful, None otherwise
        """
        self._auto_register_counter += 1
        label = pattern.format(self._auto_register_counter)

        success = self.register_face(track_id, label)
        if success:
            sample_count = 0
            if track_id in self.unknown_faces:
                sample_count = len(self.unknown_faces[track_id].embeddings)
            self._log_auto_registration(label, track_id, sample_count)
            logger.info(f"Auto-registered: {label} (track {track_id})")
            return label
        else:
            self._auto_register_counter -= 1
            return None

    def _log_auto_registration(self, label: str, track_id: int, sample_count: int):
        """Append auto-registration record to JSONL log."""
        from datetime import datetime

        self._auto_registrations_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        try:
            with open(self._auto_registrations_path, "a") as f:
                f.write(json.dumps(record) + "\n")
        except Exception as e:
            logger.error(f"Failed to log auto-registration: {e}")
